...
def determineIfWSShouldMerge(wS1, dictOwnerWSResultWStoProbs, allWordSenses):
    ...
    if closestWS[0]:
        if (zDiff > 1.0) and (closestWS[1] > 0.5):
            return closestWS[0]
    return None
    """#evaluate each wS2
    for wS2 in allWordSenses:
...
(featCONJCOUNT(tPSIO, +v_f1) ^ featUNNORMALIZEDVERB(tPSIO, +v_f11)) => wordSense(tPSIO, +v_ws)
...
featDEPENSUBARG1(tPSIO, +v_f2) ^ featUNNORMALIZEDVERB(tPSIO, +v_f11) => wordSense(tPSIO, +v_ws)
...
featDEPENSUBARG2(tPSIO, +v_f3) ^ featUNNORMALIZEDVERB(tPSIO, +v_f11) => wordSense(tPSIO, +v_ws)
...
featDEPENSUBARG3(tPSIO, +v_f4) ^ featUNNORMALIZEDVERB(tPSIO, +v_f11) => wordSense(tPSIO, +v_ws)
...
featDEPENHEADARG1(tPSIO, +v_f5) ^ featUNNORMALIZEDVERB(tPSIO, +v_f11) => wordSense(tPSIO, +v_ws)
...
featDEPENHEADARG2(tPSIO, +v_f6) ^ featUNNORMALIZEDVERB(tPSIO, +v_f11) => wordSense(tPSIO, +v_ws)
...
featDEPENHEADARG3(tPSIO, +v_f7) ^ featUNNORMALIZEDVERB(tPSIO, +v_f11) => wordSense(tPSIO, +v_ws)
...
featNEARG1(tPSIO, +v_f8) ^ featUNNORMALIZEDVERB(tPSIO, +v_f11) => wordSense(tPSIO, +v_ws)

featNEARG2(tPSIO, +v_f9) ^ featNEARG3(tPSIO, +v_f10) => wordSense(tPSIO, +v_ws)
featNEARG2(tPSIO, +v_f9) ^ featUNNORMALIZEDVERB(tPSIO, +v_f11) => wordSense(tPSIO, +v_ws)

featNEARG3(tPSIO, +v_f10) ^ featUNNORMALIZEDVERB(tPSIO, +v_f11) => wordSense(tPSIO, +v_ws)
...
def writeMLN2TrainingTuplesToFile(all_TP_WS_S_S_Fs):
    ...
    for TP_WS_S_S_F in all_TP_WS_S_S_Fs:
        incr += 1
        if getFeature("featUNNORMALIZEDVERB", TP_WS_S_S_F):
            f.write(""" featUNNORMALIZEDVERB("%s", "%s")\n """ % (TP_WS_S_S_F[0] + "###" + TP_WS_S_S_F[3] + str(incr), getFeature("featUNNORMALIZEDVERB", TP_WS_S_S_F) ) )
        else:
            f.write(""" featUNNORMALIZEDVERB("%s", "%s")\n """ % (TP_WS_S_S_F[0] + "###" + TP_WS_S_S_F[3] + str(incr), "featUNNORMALIZEDVERB_None" ) )
...
def writeMLN2TestingTuplesToFile(all_TP_WS_S_S_Fs, wSs):
    ...
    for TP_WS_S_S_F in all_TP_WS_S_S_Fs:
        if getFeature("featUNNORMALIZEDVERB", TP_WS_S_S_F):
            f.write(""" featUNNORMALIZEDVERB("%s", "%s")\n """ % (TP_WS_S_S_F[0] + "###" + TP_WS_S_S_F[3], getFeature("featUNNORMALIZEDVERB", TP_WS_S_S_F) ) )
        if getFeature("featCONJCOUNT", TP_WS_S_S_F):
...
def testMLN2(c2, addedTPs, testQueryPartToDo):
    ...
    if testQuery:
        ...
        if len(testQuery) > 125000:
            ...
            subprocess.call(r""" alchemy/bin/infer -i univ-out.mln -e univ-test.db -r univ.results -q " """ + testQuery + r""" " -lazy -lazyNoApprox 1 -maxSteps 20 -mwsMaxSteps 200 > testMLN2Output.txt """, shell=True)
...
def run(addedTPs):
    ...
    for tP in uniqueTPsBasedOnVerb(c2, addedTPs):
        ...
        while (not moveOrMerge) and (testQueryPartToDo != -1):
            ...
            moveOrMerge = moveTuplesBasedOnMLN2Clustering(c2, c3) #if there's a moveOrMerge quit doing anymore, because the database needs to be updated
    #####
    conn2.commit()
    c2.close()
    conn3.commit()
