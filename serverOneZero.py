...
def uploadBadWikipediaArticles():
    try:
        ...
        badURLs = []
        pickle.dump( badURLs, open('badWikipediaArticles.p', 'wb'))
    subprocess.call(r'ssh st1298@eros.cs.txstate.edu cat < badWikipediaArticles.p ">" badWikipediaArticles1OneZero.p  ', shell=True)
    subprocess.call(r'ssh st1298@eros.cs.txstate.edu rm badWikipediaArticlesOneZero.p  ', shell=True)
    subprocess.call(r'ssh st1298@eros.cs.txstate.edu mv badWikipediaArticles1OneZero.p badWikipediaArticlesOneZero.p ', shell=True)
...
def main():
    ...
    try:
        while(True):
            #process badURLs
            subprocess.call(r'scp st1298@eros.cs.txstate.edu:transportedBWOneZero*.p ./ ', shell=True)
            subprocess.call(r'ssh st1298@eros.cs.txstate.edu rm transportedBWOneZero*.p  ', shell=True)
            ...
            subprocess.call(r'scp st1298@eros.cs.txstate.edu:transportedTROneZero*.db ./ ', shell=True)
            subprocess.call(r'ssh st1298@eros.cs.txstate.edu rm transportedTROneZero*.db  ', shell=True)
            processedSomething = False
            for fileName in os.listdir(os.getcwd()):
                if "transportedTR" in fileName:
                    print("processing " + fileName)
            ...
            #if processedSomething:
                    subprocess.call(r'ssh st1298@eros.cs.txstate.edu cat < taxonomyRelations.db ">" taxonomyRelations1OneZero.db  ', shell=True)
                    subprocess.call(r'ssh st1298@eros.cs.txstate.edu rm taxonomyRelationsOneZero.db  ', shell=True)
                    subprocess.call(r'ssh st1298@eros.cs.txstate.edu mv taxonomyRelations1OneZero.db taxonomyRelationsOneZero.db ', shell=True)
            time.sleep(2)
            conn2 = sqlite3.connect("taxonomyRelations.db")
            c2 = conn2.cursor(cursor)
            c2.execute("pragma foreign_keys = ON")
