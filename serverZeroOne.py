...
def uploadBadWikipediaArticles():
    try:
        ...
        badURLs = []
        pickle.dump( badURLs, open('badWikipediaArticles.p', 'wb'))
    subprocess.call(r'ssh st1298@eros.cs.txstate.edu cat < badWikipediaArticles.p ">" badWikipediaArticles1ZeroOne.p  ', shell=True)
    subprocess.call(r'ssh st1298@eros.cs.txstate.edu rm badWikipediaArticlesZeroOne.p  ', shell=True)
    subprocess.call(r'ssh st1298@eros.cs.txstate.edu mv badWikipediaArticles1ZeroOne.p badWikipediaArticlesZeroOne.p ', shell=True)
...
def main():
    ...
    try:
        while(True):
            #process badURLs
            subprocess.call(r'scp st1298@eros.cs.txstate.edu:transportedBWZeroOne*.p ./ ', shell=True)
            subprocess.call(r'ssh st1298@eros.cs.txstate.edu rm transportedBWZeroOne*.p  ', shell=True)
            ...
            subprocess.call(r'scp st1298@eros.cs.txstate.edu:transportedTRZeroOne*.db ./ ', shell=True)
            subprocess.call(r'ssh st1298@eros.cs.txstate.edu rm transportedTRZeroOne*.db  ', shell=True)
            processedSomething = False
            for fileName in os.listdir(os.getcwd()):
                if "transportedTR" in fileName:
                    print("processing " + fileName)
            ...
            #if processedSomething:
                    subprocess.call(r'ssh st1298@eros.cs.txstate.edu cat < taxonomyRelations.db ">" taxonomyRelations1ZeroOne.db  ', shell=True)
                    subprocess.call(r'ssh st1298@eros.cs.txstate.edu rm taxonomyRelationsZeroOne.db  ', shell=True)
                    subprocess.call(r'ssh st1298@eros.cs.txstate.edu mv taxonomyRelations1ZeroOne.db taxonomyRelationsZeroOne.db ', shell=True)
            time.sleep(2)
            conn2 = sqlite3.connect("taxonomyRelations.db")
            c2 = conn2.cursor(cursor)
            c2.execute("pragma foreign_keys = ON")
