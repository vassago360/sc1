...
def enoughURLsToWorkWith(urls, exemplarRelationInfo):
    if "backUpTaxonomy.db" in os.listdir(os.getcwd()): #an indication that taxonomy formation is running for the first time
        ...
        if (distinctZeugmaReductionSituations >= 5) or (len(urls) > 0): #important during evalInput.  We want a consistant amount of wordSenses that a tP will be compared to (a parameter of the program so-to-speak)
